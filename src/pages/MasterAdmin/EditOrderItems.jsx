                                        ...
                                        <div key={pid} className="border border-gray-200 rounded-xl p-4 bg-gradient-to-r from-gray-50 to-white hover:shadow-md transition-all duration-200">
                                            <div className="flex items-center justify-between gap-3 mb-3">
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <button
                                                        onClick={async () => {
                                                            const emptyIdx = editRows.findIndex(r => !r.product_id);
                                                            if (emptyIdx === -1) {
                                                                addEmptyRow();
                                                                setTimeout(async () => {
                                                                    const newIdx = editRows.length;
                                                                    updateRow(newIdx, { product_id: pid });
                                                                    if (p.price ?? p.unit_price) updateRow(newIdx, { unit_price: p.price ?? p.unit_price });
                                                                    const details = await fetchProductDetails(pid).catch(() => null);
                                                                    if (details && typeof details.price !== "undefined") updateRow(newIdx, { unit_price: details.price });
                                                                    fetchBatches(pid).catch(() => {});
                                                                }, 60);
                                                            } else {
                                                                updateRow(emptyIdx, { product_id: pid });
                                                                if (p.price ?? p.unit_price) updateRow(emptyIdx, { unit_price: p.price ?? p.unit_price });
                                                                fetchProductDetails(pid).then(details => {
                                                                    if (details && typeof details.price !== "undefined") updateRow(emptyIdx, { unit_price: details.price });
                                                                });
                                                                fetchBatches(pid).catch(() => {});
                                                            }
                                                        }}
